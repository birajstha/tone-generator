<script>
  ...
  const RELEASE_TIME = 0.015;
  const GATE_TIME = 0.004;
  ...
  function startTone() {
    ...
    if (pulseMode) {
      ...
      gainNode.gain.setValueAtTime(0, now);
      gainNode.gain.linearRampToValueAtTime(volume / 100, now + ATTACK_TIME);
    }
  }

  let pulseInterval = null;
  let pulseOffTimeout = null;

  function startPulse() {
    if (pulseInterval) clearInterval(pulseInterval);
    if (pulseOffTimeout) {
      clearTimeout(pulseOffTimeout);
      pulseOffTimeout = null;
    }
    ...
    function pulse() {
      ...
      gainNode.gain.linearRampToValueAtTime(volume / 100, now + GATE_TIME);

      pulseOffTimeout = setTimeout(() => {
        if (!isPlaying || !gainNode || !audioCtx) return;
        const offNow = audioCtx.currentTime;
        gainNode.gain.cancelScheduledValues(offNow);
        gainNode.gain.setValueAtTime(gainNode.gain.value, offNow);
        gainNode.gain.linearRampToValueAtTime(0, offNow + GATE_TIME);
      }, Math.max(0, onTime - GATE_TIME * 1000));
    }
  ...
  function stopTone() {
    ...
    const localOscillator = oscillator;
    ...
    localGain.gain.linearRampToValueAtTime(0, now + RELEASE_TIME);
    ...
    setTimeout(() => {
      if (localOscillator) {
        try { localOscillator.stop(); } catch(e) {}
      }
      if (localGain) localGain.disconnect();
      if (localAudioCtx) localAudioCtx.close();
    }, Math.ceil(RELEASE_TIME * 1000) + 10);
  }

  function updateOscillator() {
    if (oscillator && isPlaying && audioCtx) {
  ...
  function updateVolume() {
    if (gainNode && isPlaying && audioCtx && !pulseMode) {
      const now = audioCtx.currentTime;
      gainNode.gain.setTargetAtTime(volume / 100, now, 0.01);
    }
  }
  ...
  $: if (gainNode && isPlaying && !pulseMode) {
    if (audioCtx) {
      gainNode.gain.setTargetAtTime(volume / 100, audioCtx.currentTime, 0.01);
    }
  }
